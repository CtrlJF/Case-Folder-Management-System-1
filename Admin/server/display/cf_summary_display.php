...
if(isset($_SESSION['id_no'])) {
    ...
    $query = "
            SELECT 
            ua.acc_id AS user_acc_id, ua.fname, ua.mname, ua.lname,
        ...
        WHERE ua.hhid = :hhid
        ";
    /* $hhid = 123456789123412344; */
    // Execute query
    $statement = $pdo->prepare($query);
    $statement->bindParam(':hhid', $hhid, PDO::PARAM_INT);
    $statement->execute();
    ...
    foreach ($result as $row) {
        ...
        $userInfo = $row['fname'] . ' ' . $row['mname'] . ' ' . $row['lname'];
