...
ORDER BY
    ...
    $barangayGroups1 = [];
    // Initialize an array to hold all upload dates
    $uploadYears = [];
    ...
    foreach ($outputs as $row) {
        ...
        // Extract and aggregate upload years
        if ($row['upload']) {
            $uploadYear = date('Y', strtotime($row['upload']));
            if (!in_array($uploadYear, $uploadYears)) {
                $uploadYears[] = $uploadYear;
            }
        }
    }
    ...
    sort($uploadYears);
    $years = $uploadYears;
    // Output the $uploadYears array to check the result
    /* echo '<pre>';
    print_r($uploadYears);
    echo '</pre>'; */
