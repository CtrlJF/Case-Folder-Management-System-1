...
foreach ($completedUsers as $acc_id) {
    ...
    WHERE
        ua.status IN ('active', 'inactive', 'completed') -- Exclude 'pending' status
    ORDER BY
        ua.fname;
    ";
    
    /* $sql = "SELECT hhid, fname, mname, lname, phone_number, barangay, purok, user_set, phylsis_id, status FROM user_account WHERE status != 'pending' ORDER BY lname"; */
