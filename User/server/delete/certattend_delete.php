...
if (isset($_SESSION['acc_id']) && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $pdo->beginTransaction();
         ...
         $sql = "SELECT img FROM user_certificate WHERE id = :userId";
         ...
         if ($row) {
            $baseDir = '../../../uploads/certattend/';
            $fileName = $row['img'];
            
            $filePath = $baseDir . $fileName;

            // Prepare the DELETE statement
            $sql = "DELETE FROM user_certificate WHERE id = :userId";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':userId', $userNID, PDO::PARAM_INT);
            ...
            if ($stmt->execute()) {
                ...
                echo "window.location='../../certattendance.php';</script>";
                exit();
            } else {
    ...
    echo "<script>window.location='../../certattendance.php';</script>";
}
