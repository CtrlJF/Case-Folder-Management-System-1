...
if (isset($_SESSION['acc_id']) && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $pdo->beginTransaction();
         ...
         $sql = "SELECT img FROM user_mdr WHERE id = :userId";
         ...
         if ($row) {
            ...
            $sql = "DELETE FROM user_mdr WHERE id = :userId";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':userId', $userNID, PDO::PARAM_INT);
            ...
            if ($stmt->execute()) {
                ...
                echo "window.location='../../mdr.php';</script>";
                exit();
            } else {
    ...
    echo "<script>window.location='../../mdr.php';</script>";
}
