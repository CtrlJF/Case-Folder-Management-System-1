...
if (isset($_SESSION['acc_id']) && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $pdo->beginTransaction();
         ...
         $sql = "SELECT img FROM user_grade_cards WHERE id = :userId";
         ...
         if ($row) {
            $baseDir = '../../../uploads/gradecards/';
            $fileName = $row['img'];
            
            $filePath = $baseDir . $fileName;

            // Prepare the DELETE statement
            $sql = "DELETE FROM user_grade_cards WHERE id = :userId";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':userId', $userNID, PDO::PARAM_INT);
            ...
            if ($stmt->execute()) {
                ...
                echo "window.location='../../gradecards.php';</script>";
                exit();
            } else {
    ...
    echo "<script>window.location='../../gradecards.php';</script>";
}
