...
if (isset($_SESSION['acc_id']) && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $pdo->beginTransaction();
         ...
         $sql = "SELECT img FROM user_kasabutan WHERE id = :userId";
         ...
         if ($row) {
            $baseDir = '../../../uploads/kasabutan/';
            $fileName = $row['img'];
            
            $filePath = $baseDir . $fileName;

            // Prepare the DELETE statement
            $sql = "DELETE FROM user_kasabutan WHERE id = :userId";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':userId', $userNID, PDO::PARAM_INT);
    ...
    echo "<script>window.location='../../kasabutan.php';</script>";
}
