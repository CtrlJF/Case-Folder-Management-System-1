...
if (isset($_SESSION['acc_id']) && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $sql = "SELECT img FROM user_birth_certificate WHERE id = :userId";
        ...
        if ($row) {
           $baseDir = '../../../uploads/birthcert/';
           $fileName = $row['img'];
           
           $filePath = $baseDir . $fileName;

           // Prepare the DELETE statement
           $sql = "DELETE FROM user_birth_certificate WHERE id = :userId";
           $stmt = $pdo->prepare($sql);
           $stmt->bindParam(':userId', $userNID, PDO::PARAM_INT);
           ...
           if ($stmt->execute()) {
               ...
               echo "window.location='../../birthcert.php';</script>";
               exit();
           } else {
    ...
    echo "<script>window.location='../../birthcert.php';</script>";
}
