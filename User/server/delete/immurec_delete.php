...
if (isset($_SESSION['acc_id']) && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $pdo->beginTransaction();
         ...
         $sql = "SELECT img FROM user_immunization_record WHERE id = :userId";
         ...
         if ($row) {
            $baseDir = '../../../uploads/immurec/';
            $fileName = $row['img'];
            
            $filePath = $baseDir . $fileName;

            // Prepare the DELETE statement
            $sql = "DELETE FROM user_immunization_record WHERE id = :userId";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':userId', $userNID, PDO::PARAM_INT);
            ...
            if ($stmt->execute()) {
                ...
                echo "window.location='../../immurec.php';</script>";
                exit();
            } else {
    ...
    echo "<script>window.location='../../immurec.php';</script>";
}
