...
require_once 'server/displays/cashcard_display.php';
...
<head>
    ...
    <script src="functions/cashcard.js"></script>
...
<body>
                    ...
                    <div class="row">
                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                            <div class="page-header">
                                <h2 class="pageheader-title">Cash Card</h2>
                                    ...
                                    <?php
                                        if ($stmt->rowCount() > 0) {
                                            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                                ...
                                                if (!empty($row['img'])) {
                                                    $filePath = '../uploads/cashcard/' . $row['img'];
                                                    ...
                                                    if ($extension == 'pdf') {
                                                        ...
                                                        echo "<img src='../uploads/cashcard/{$row['img']}' alt='img' class='cover-image'>";
                                                    }
                                                } else {
                                                ...
                                                echo "<input id='file-input-{$row['id']}' type='file' style='display: none;' accept='image/jpeg,image/png,application/pdf' data-user-cash-card='{$row['id']}' onchange='handleFileSelect(event)'>";    
                                                

                                                if (!empty($row['img']) && file_exists("../uploads/cashcard/{$row['img']}")) {
                                                    echo "<a href='#' class='btn btn-info mr-2 printButton' data-img-src='../uploads/cashcard/{$row['img']}'> Print </a>";
                                                } else {
                                                ...
                                                echo "<button type='button' class='btn btn-danger delButton' data-toggle='modal' data-target='#delForm' data-user-cash-card='{$row['id']}'>Delete</button>";
                                                echo "</div>";
                                                ...
                                                echo "<a href='#' data-toggle='modal' data-target='#remarks' data-type='Cash_Card' data-row-id='{$row['id']}'>Remarks</a>";
                                                echo "</div>";
                                                echo "</div>";
                                                echo "</div>";
                                                echo "</div>";
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="server/create/cashcard_create.php" method="post">
                            <div class="form-group">
                                <label for="name">Card name:</label>
                                <input type="text" id="name" name="name" class="form-control" required placeholder="Ex. Cash Card">
                            </div>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body text-center">
                        <form action="server/delete/cashcard_delete.php" method="post">
                            <div class="form-group">
    ...
    $(document).ready(function() {
        ...
        $('#delForm').on('show.bs.modal', function(event) {
            ...
            var userId = button.data('user-cash-card'); // Extract info from data-* attributes
            console.log(userId);
